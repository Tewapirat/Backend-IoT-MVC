...
import { IController } from "@/common/interfaces/controller.onterface";
...
export class DeviceController implements IController{
    ...
    private service: DeviceService = Container.get(DeviceService)
    ...
    public delete = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            res.status(201).json({message:'delete',data: result})
        } catch (error) {
            next(error)
